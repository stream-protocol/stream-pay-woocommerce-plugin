<?php

/**
 * @wordpress-plugin
 * Plugin Name:             StreamPay Gateway for WooCommerce
 * Plugin URI:              https://github.com/stream-protocol/streampay-woocommerce-plugin
 * Description:             Cryptocurrency Payment Gateway.
 * Version:                 1.0.1
 * Author:                  Stream Protocol / Stream Pay
 * License:                 GPLv2 or later
 * License URI:             http://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain:             wc-stream-pay-gateway
 * Domain Path:             /i18n/languages/
 */

/**
 * Exit if accessed directly.
 */
if (! defined('ABSPATH'))
{
    exit();
}

if (version_compare(PHP_VERSION, '7.1', '>=')) {
    ini_set('precision', 10);
    ini_set('serialize_precision', 10);
}

if (! defined('STREAMPAY_FOR_WOOCOMMERCE_PLUGIN_DIR')) {
    define('STREAMPAY_FOR_WOOCOMMERCE_PLUGIN_DIR', dirname(__FILE__));
}

if (! defined('STREAMPAY_FOR_WOOCOMMERCE_ASSET_URL')) {
    define('STREAMPAY_FOR_WOOCOMMERCE_ASSET_URL', plugin_dir_url(__FILE__));
}

if (! defined('STREAMPAY_VERSION_PFW')) {
    define('STREAMPAY_VERSION_PFW', '1.0.1');
}

if (! defined('STREAMPAY_SUPPORT_EMAIL')) {
    define('STREAMPAY_SUPPORT_EMAIL', 'contact@streamprotocol.org');
}

if (! defined('STREAMPAY_ICON_URL')) {
    define('STREAMPAY_ICON_URL', 'data:image/png;base64,');
}

if (! defined('STREAMPAY_PAY_LINK')) {
    define('STREAMPAY_PAY_LINK', 'https://streampay.me/pay');
}

if (! defined('STREAMPAY_API_URL')) {
    define('STREAMPAY_API_URL', 'https://api.streampay.me/v1');
}

if (! defined('STREAMPAY_SETTLEMENT_ASSETS_API')) {
    define('STREAMPAY_SETTLEMENT_ASSETS_API', STREAMPAY_API_URL . '/setting/settlement_assets');
}

if (! defined('STREAMPAY_QUOTE_ASSETS_API')) {
    define('STREAMPAY_QUOTE_ASSETS_API', STREAMPAY_API_URL . '/setting/quote_assets');
}

if (! defined('STREAMPAY_MIXINUUID_API')) {
    define('STREAMPAY_MIXINUUID_API', STREAMPAY_API_URL . '/user/mixin_uuid');
}

if (! defined('STREAMPAY_MULTISIG_API')) {
    define('STREAMPAY_MULTISIG_API', STREAMPAY_API_URL . '/multisig');
}

if (! defined('STREAMPAY_ASSETS_EXPIRE_SECONDS')) {
    define('STREAMPAY_ASSETS_EXPIRE_SECONDS', 600);
}

if (! defined('STREAMPAY_PAYMENTS_RESULT')) {
    define('STREAMPAY_PAYMENTS_RESULT', STREAMPAY_API_URL . '/payments_result');
}

/**
 * Add the gateway to WC Available Gateways
 *
 * @since 1.0.0
 * @param array $gateways all available WC gateways
 * @return array $gateways all WC gateways + offline gateway
 */
if (! function_exists('wc_stream_pay_add_to_gateways')) {
    function wc_STREAMPAY_add_to_gateways($gateways)
    {
        if (! in_array('WC_Gateway_stream_pay ', $gateways)) {
            $gateways[] = 'WC_Gateway_stream_pay ';
        }

        return $gateways;
    }
}
add_filter( 'woocommerce_payment_gateways', 'wc_stream_pay_add_to_gateways' );

/**
 * Adds plugin page links
 *
 * @since 1.0.0
 * @param array $links all plugin links
 * @return array $links all plugin links + our custom links (i.e., "Settings")
 */
if (! function_exists('wc_stream_pay_gateway_plugin_links')) {
    function wc_stream_pay_gateway_plugin_links($links)
    {

        $plugin_links = [
            '<a href="' . esc_url(admin_url('admin.php?page=wc-settings&tab=checkout&section=stream_pay_gateway')) . '">' . esc_html(__('Configure', 'wc-stream-pay-gateway')) . '</a>',
            '<a href="mailto:' . esc_html(STREAMPAY_SUPPORT_EMAIL) . '">' . esc_html(__('Email', 'wc-stream-pay-gateway')) . '</a>'
        ];

        return array_merge($plugin_links, $links);
    }
}
add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'wc_stream_pay_gateway_plugin_links' );

if (! function_exists('add_cron_every_minute_interval')) {
    function add_cron_every_minute_interval($schedules)
    {
        if (! isset($schedules['every_minute'])) {
            $schedules['every_minute'] = [
                'interval' => 60,
                'display'  => esc_html__('Every minute'),
            ];
        }

        return $schedules;
    }
}
add_filter( 'cron_schedules', 'add_cron_every_minute_interval' );

/**
 * StreamPay Payment Gateway
 *
 * @class 		WC_Gateway_Stream_Pay 
 * @extends		WC_Payment_Gateway
 * @version		1.0.0
 * @package		WooCommerce/Classes/Payment
 * @author 		Echo
 */
add_action('plugins_loaded', 'wc_stream-pay_gateway_init', 11);
function wc_stream_pay_gateway_init()
{

    if (! class_exists('WC_Payment_Gateway')) {
        return;
    }

    add_action('check_payments_result_cron_hook', ['WC_Gateway_stream_pay ', 'check_payments_result'], 10, 1);

    class WC_Gateway_stream_pay  extends WC_Payment_Gateway
    {
        /**
         * Constructor for the StreamPay Payment Gateway.
         *
         * @access public
         * @return void
         */
        public function __construct()
        {
            global $woocommerce;
            $this->id                 = 'stream_pay_gateway';
            $this->icon               = apply_filters('woocommerce_stream_pay_icon', STREAMPAY_ICON_URL);
            $this->has_fields         = false;
            $this->method_title       = esc_html(__('StreamPay Payment Gateway', 'wc-gateway-stream-pay'));
            $this->method_description = esc_html(__( 'Allows Cryptocurrency payments via StreamPay', 'wc-stream-pay-gateway' ));

            // Load the settings.
            $this->init_form_fields();
            $this->init_settings();

            // Define user set variables
            $this->title                   = $this->get_option('title');
            $this->description             = $this->get_option('description');
            $this->instructions            = $this->get_option( 'instructions');
            $this->mixin_id                = $this->get_option('mixin_id');
            $this->payee_uuid              = $this->get_option('payee_uuid');
            $this->store_name              = $this->get_option('store_name');
            $this->settlement_asset_id     = $this->get_option('settlement_asset_id');
            $this->invoice_prefix          = $this->get_option('invoice_prefix', 'WORDPRESS-WC-');
            $this->debug                   = $this->get_option('debug', false);

            // Logs
            $this->log = new WC_Logger();

            // Actions
            add_filter('woocommerce_available_payment_gateways', [$this, 'check_is_valid_for_use'], 1, 1);
            add_action('woocommerce_page_wc-settings', [$this, 'is_valid_for_use'], 1);
            add_action('woocommerce_update_options_payment_gateways_' . $this->id, [$this, 'process_admin_options']);
            add_action('woocommerce_thank_you_' . $this->id, [ $this, 'thank_you_page' ] );
            add_action('woocommerce_api_WC_Gateway_stream_pay ', [$this, 'stream_pay_callback']);
            add_filter('woocommerce_settings_api_sanitized_fields_' . $this->id, [$this, 'deal_options'], 1, 1);

            // Customer Emails
            add_action( 'woocommerce_email_before_order_table', [ $this, 'email_instructions' ], 10, 3 );
        }

        /**
         * Initialize StreamPay Gateway Settings Form Fields
         *
         * @access public
         * @return void
         */
        function init_form_fields()
        {
            $this->form_fields = apply_filters( 'wc_offline_form_fields', [
                'enabled' => [
                    'title'   => esc_html(__('Enable/Disable', 'wc-stream-pay-gateway')),
                    'type'    => 'checkbox',
                    'label'   => esc_html(__('Enable StreamPay', 'wc-stream-pay-gateway')),
                    'default' => 'yes',
                ],
                'title' => [
                    'title'       => esc_html(__('Title', 'wc-stream-pay-gateway')),
                    'type'        => 'text',
                    'description' => esc_html(__('This controls the title which the user sees during checkout.', 'wc-stream-pay-gateway')),
                    'default'     => esc_html(__('StreamPay', 'wc-stream-pay-gateway')),
                ],
                'description' => [
                    'title'       => esc_html(__('Description', 'wc-stream-pay-gateway')),
                    'type'        => 'textarea',
                    'description' => esc_html(__('This controls the description which the user sees during checkout.', 'wc-stream-pay-gateway')),
                    'default'     => esc_html(__('Expand your payment options with StreamPay! BTC, ETH, LTC, SOL, and other cryptocurrencies!', 'wc-stream-pay-gateway')),
                ],
                'mixin_id' => [
                    'title'       => esc_html(__('Mixin Id ', 'wc-stream-pay-gateway')),
                    'type'        => 'text',
                    'description' => __('(<strong style="color: red">Before setting the Mixin Id, the StreamPay robot 7000104220 must be added to the Stream wallet as a contact</strong>) This controls the mixin id or multisig group (minxinid_1|minxinid_2|minxinid_3|threshold)', 'wc-stream-pay-gateway'),
                ],
                'payee_uuid' => [
                    'title'             => esc_html(__('Payee Uuid ', 'wc-stream-pay-gateway')),
                    'type'              => 'text',
                    'description'       => esc_html(__('This controls the assets payee uuid.', 'wc-stream-pay-gateway')),
                    'custom_attributes' => ['readonly' => true]
                ],
                'settlement_asset_id' => [
                    'title'       => esc_html(__('Settlement Asset ', 'wc-stream-pay-gateway')),
                    'type'        => 'select',
                    'description' => esc_html(__('This controls the assets received by the merchant.', 'wc-stream-pay-gateway')),
                    "options"     => $this->get_settlement_asset_lists(),
                ],
                'instructions' => [
                    'title'       => esc_html(__( 'Instructions', 'wc-gateway-gateway' )),
                    'type'        => 'textarea',
                    'description' => esc_html(__( 'The contents of this option are displayed on order received page and order email', 'wc-stream-pay-gateway' )),
                    'default'     => esc_html('Thanks for your using STREAMPAY Payment !'),
                ],
                'store_name' => [
                    'title'       => esc_html(__('Store Name', 'wc-stream-pay-gateway')),
                    'type'        => 'text',
                    'description' => esc_html(__("(Optional) This option is useful when you have multiple stores, and want to view each store's payment history in STREAMPAY independently.", 'wc-stream-pay-gateway')),
                ],
                'debug' => [
                    'title'       => esc_html(__('Debug', 'wc-stream-pay-gateway')),
                    'type'        => 'text',
                    'description' => esc_html(__('(this will Slow down website performance) Send post data to debug', 'wc-stream-pay-gateway')),
                ]
            ] );
        }

        /**
         * Output for the order received page.
         */
        public function thankyou_page() {
            if ( $this->instructions ) {
                echo wpautop( wptexturize( $this->instructions ) );
            }
        }

        /**
         * Add content to the WC emails.
         *
         * @access public
         * @param WC_Order $order
         * @param bool $sent_to_admin
         * @param bool $plain_text
         */
        public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
            if ( $this->instructions && $this->id === $order->get_payment_method() ) {
                echo wpautop(wptexturize($this->instructions));
            }
        }

        /**
         * Process the payment and return the result
         *
         * @access public
         * @param int $order_id
         * @return array
         */
        function process_payment($order_id)
        {
            $order        = wc_get_order($order_id);
            $redirect_url = $this->generate_STREAMPAY_url($order);

            if ( ! wp_next_scheduled( 'check_payments_result_cron_hook', [$order_id]) ) {
                wp_schedule_event(time(), 'every_minute', 'check_payments_result_cron_hook', [$order_id]);
            }

            return [
                'result'   => 'success',
                'redirect' => $redirect_url
            ];
        }

        /**
         * Generate the StreamPay button link
         *
         * @access public
         * @param mixed $order_id
         * @param mixed $order
         * @return string
         */
        function generate_STREAMPAY_url($order)
        {
            global $woocommerce;

            if ($order->status != 'completed' && get_post_meta($order->id, '"Stream" Payment complete', true) != 'Yes') {
                $order->add_order_note(esc_html('Customer is being redirected to StreamPay...'));
            }

            $amount = number_format($order->get_total(), 8, '.', '');
            $rev    = bccomp($amount, 0, 8);

            if($rev === 0){
                $order->update_status('completed', 'The order amount is zero.');

                return $this->get_return_url($order);
            }elseif ($rev === -1){
                throw new Exception("The order amount is incorrect, please contact customer");
            }

            $STREAMPAY_args = $this->get_STREAMPAY_args($order);
            $STREAMPAY_adr  = STREAMPAY_PAY_LINK . '?' . http_build_query($STREAMPAY_args);

            return $STREAMPAY_adr;
        }

        /**
         * Get StreamPay Args
         *
         * @access public
         * @param mixed $order
         * @return array
         */
        function get_STREAMPAY_args($order)
        {
            global $woocommerce;
            $STREAMPAY_args = [
                'payeeId'           => $this->payee_uuid,
                'orderId'           => $this->invoice_prefix . $order->get_order_number(),
                'tagname'           => $this->store_name,
                'settlementAssetId' => $this->settlement_asset_id,
                'quoteAssetId'      => strtolower($order->get_currency()),
                'quoteAmount'       => number_format($order->get_total(), 8, '.', ''),
                'returnTo'          => $this->get_return_url($order),
                'callbackUrl'       => site_url() . "/?wc-api=WC_Gateway_stream_pay ",
                'channel'           => 'wordpress_plugin'
            ];

            if(get_option('woocommerce_manage_stock') === 'yes'){
                $woocommerce_hold_stock_minutes  = get_option('woocommerce_hold_stock_minutes') ?: 1;
                $woocommerce_hold_stock_minutes  = $woocommerce_hold_stock_minutes > 240 ? 240 : $woocommerce_hold_stock_minutes;
                $created_time                    = strtotime($order->get_date_created());
                $STREAMPAY_args['expiredTimestamp'] = $created_time + $woocommerce_hold_stock_minutes * 60 - 30;

                $expiry_message = sprintf(
                    __( 'Sorry, your session has expired. <a href="%s" class="wc-backward">Return to shop</a>', 'woocommerce' ),
                    esc_url( wc_get_page_permalink( 'shop' ) )
                );

                if(! $order->has_status('pending')){
                    throw new Exception($expiry_message);
                }

                if($STREAMPAY_args['expiredTimestamp'] <= time()){
                    if($order->has_status('pending')) {
                        $order->update_status('cancelled', 'Unpaid order cancelled - time limit reached');
                    }
                    throw new Exception($expiry_message);
                }
            }

            $STREAMPAY_args = apply_filters('woocommerce_STREAMPAY_args', $STREAMPAY_args);

            return $STREAMPAY_args;
        }

        function chenk_is_valid_for_use($_available_gateways)
        {
            if(! $this->is_valid_for_use()){
                unset($_available_gateways[$this->id]);
            }
            return $_available_gateways;
        }

        /**
         * Check if this gateway is enabled and available in the user's country
         *
         * @access public
         * @return bool
         */
        function is_valid_for_use()
        {
            $asset_lists = $this->get_quote_asset_lists();
            $currency    = get_woocommerce_currency();

            if(! in_array(strtolower($currency), $asset_lists)){
                $this->enabled = false;

                return false;
            }

            return true;
        }

        /**
         * Admin Panel Options
         * - Options for bits like 'title' and availability on a country-by-country basis
         * @since 1.0.0
         */
        public function admin_options()
        {
            ?>
            <h3><?php _e('StreamPay', 'WooCommerce'); ?></h3>
            <p><?php _e('Completes checkout via StreamPay', 'WooCommerce'); ?></p>

            <?php if ($this->enabled) { ?>

            <table class="form-table">
                <?php
                $this->generate_settings_html();
                ?>
            </table>
            <!--/.form-table-->

        <?php } else { ?>
            <div class="inline error">
                <p><strong><?php _e('Stream Gateway Disabled', 'woocommerce'); ?></strong>: <?php _e('StreamPay doesn´t not support your store currency.', 'WooCommerce'); ?></p>
            </div>
        <?php }

        }

        /**
         * @access public
         * @param array $posted
         * @return void
         */
        function STREAMPAY_callback()
        {
            ob_start();
            global $woocommerce;

            $request_json         = file_get_contents('php://input');
            $request_data         = json_decode($request_json, true);
            $payments_result_data = $this->get_payments_result($request_data["orderId"], $request_data["payeeId"]);
            $valid_order_id       = str_replace($this->invoice_prefix, '', $request_data["orderId"]);
            $result               = $this->update_order_status($valid_order_id, $payments_result_data);

            ob_clean();
            wp_send_json([ 'code' => $result['code']], $result['status']);
        }

        static function check_payments_result($order_id)
        {
            $STREAMPAY_gateway        = (new self());
            $payments_result_data = $STREAMPAY_gateway->get_payments_result($STREAMPAY_gateway->invoice_prefix . $order_id, $STREAMPAY_gateway->payee_uuid);
            $STREAMPAY_gateway->update_order_status($order_id, $payments_result_data);
        }

        function update_order_status($order_id, $payments_result_data)
        {
            $order  = new WC_Order($order_id);
            $result = ['code' => 'FAIL', 'status' => 500];

            $status_before_update = $order->get_status();

            if($payments_result_data["status"] == "pending" && $status_before_update == 'pending') {
                $order->update_status('processing', 'Order is processing.');
            } elseif($payments_result_data["status"] == "success" && in_array($status_before_update, ['pending', 'processing'])) {
                $order->update_status('completed', 'Order has been paid.');
                $result = ['code' => 'SUCCESS', 'status' => 200];
            } elseif($payments_result_data["status"] == "failed") {
                $order->update_status('cancelled', "Order has been cancelled, reason: {$payments_result_data['failureReason']}.");
            }

            if (! $order->has_status(['pending', 'processing'])){
                wp_clear_scheduled_hook('check_payments_result_cron_hook', [$order_id]);
            }

            $this->debug_post_out(
                'update_order_status',
                [
                    'payments_result_data'              => $payments_result_data,
                    'order_status_before_update'        => $status_before_update,
                    'order_status_after_update'         => $order->get_status()
                ]
            );

            return $result;
        }

        function get_payments_result($order_id, $payee_uuid)
        {
            $response             = wp_remote_get(STREAMPAY_PAYMENTS_RESULT . "?orderId={$order_id}&payeeId={$payee_uuid}");
            $payments_result_data = wp_remote_retrieve_body($response);

            return  json_decode($payments_result_data, true)['data'];
        }

        function get_quote_asset_lists()
        {
            $key               = 'STREAMPAY_quote_asset_lists';
            $quote_asset_lists = get_option($key);

            if(isset($quote_asset_lists['expire_time']) && $quote_asset_lists['expire_time'] > time()){
                return $quote_asset_lists['data'];
            }

            $response      = wp_remote_get(STREAMPAY_QUOTE_ASSETS_API);
            $response_data = wp_remote_retrieve_body($response);
            $response_data = json_decode($response_data, true)['data'] ?? [];
            $lists         = array_column($response_data, 'assetId');

            if(! empty($lists)) {
                $quote_asset_lists = $lists;
                update_option($key, ['data' => $lists, 'expire_time' => time() + STREAMPAY_ASSETS_EXPIRE_SECONDS]);
            }

            return $quote_asset_lists;
        }

        function get_settlement_asset_lists()
        {
            $key                    = 'STREAMPAY_settlement_asset_lists';
            $settlement_asset_lists = get_option($key);

            if(isset($settlement_asset_lists['expire_time']) && $settlement_asset_lists['expire_time'] > time()){
                return $settlement_asset_lists['data'];
            }

            $response      = wp_remote_get(STREAMPAY_SETTLEMENT_ASSETS_API);
            $response_data = wp_remote_retrieve_body($response);
            $response_data = json_decode($response_data, true)['data'] ?? [];

            $lists = [];

            foreach ($response_data as $asset) {
                $item_asset_id         = esc_attr($asset['assetId']);
                $lists[$item_asset_id] = empty($asset['network']) ? esc_html($asset['symbol']) : esc_html($asset['symbol'] . ' - ' . $asset['network']);
            }

            if(! empty($lists)) {
                $settlement_asset_lists = $lists;
                update_option($key, ['data' => $lists, 'expire_time' => time() + STREAMPAY_ASSETS_EXPIRE_SECONDS]);
            }

            return $settlement_asset_lists;
        }

        function getRandomString($length = 8)
        {
            $captcha = '';

            for($i = 0;$i < $length; $i++){
                $captcha .= chr(mt_rand(65, 90));
            }

            return $captcha;
        }

        function deal_options($settings)
        {
            if(empty($settings['title'])){
                WC_Admin_Settings::add_error(esc_html("Title is required"));
            }

            if(empty($settings['mixin_id'])){
                WC_Admin_Settings::add_error(esc_html("Mixin Id is required"));
            }

            if(empty($settings['settlement_asset_id'])){
                WC_Admin_Settings::add_error(esc_html("Settlement asset is required"));
            }

            if(! empty($settings['store_name']) && ! preg_match('/^[a-zA-Z0-9]+$/u', $settings['store_name'])){
                WC_Admin_Settings::add_error(esc_html("Store Name must only contain letters and numbers"));
            }

            $mixin_id = $settings['mixin_id'] ?? '';

            if($mixin_id) {
                if (strpos($mixin_id, '|') !== false) {
                    $receiver_mixin_ids = explode('|', $mixin_id);
                    $threshold          = end($receiver_mixin_ids);
                    array_pop($receiver_mixin_ids);
                    $receiver_uuids = [];

                    if(count($receiver_mixin_ids) < $threshold){
                        WC_Admin_Settings::add_error(esc_html("Multisig threshold not more than the count of receivers"));
                    }

                    foreach ($receiver_mixin_ids as $mixin_id) {
                        $receiver_uuids[] = esc_html($this->get_mixin_uuid($mixin_id));
                    }
                    $settings['payee_uuid'] = esc_html($this->get_multisig_id($receiver_uuids, $threshold));
                } else {
                    $settings['payee_uuid'] = esc_html($this->get_mixin_uuid($mixin_id));
                }
            }else{
                $settings['payee_uuid'] = '';
            }

            if(empty($settings['payee_uuid'])){
                WC_Admin_Settings::add_error(esc_html("Payee uuid was not obtained, please try again later. (Make sure you have added StreamPay robot 7000104220 as a contact in your Stream wallet)"));
            }

            if(empty($settings['invoice_prefix'])) {
                $settings['invoice_prefix'] = esc_html($this->getRandomString());
            }

            return $settings;
        }

        function get_mixin_uuid($mixin_id)
        {
            $response               = wp_remote_get(STREAMPAY_MIXINUUID_API . "/{$mixin_id}");
            $response_data          = wp_remote_retrieve_body($response);
            $response_data          = json_decode($response_data, true)['data'] ?? [];

            if(empty($response_data['payeeId'])){
                WC_Admin_Settings::add_error(esc_html("Mixin uuid was not obtained, please try again later"));
            }

            return $response_data['payeeId'] ?? '';
        }

        function get_multisig_id($receiver_uuids, $threshold)
        {
            $response               = wp_remote_post( STREAMPAY_MULTISIG_API, [
                'body' => [
                    'receivers' => $receiver_uuids,
                    'threshold' => $threshold
                ]
            ]);

            $response_data          = wp_remote_retrieve_body($response);
            $response_data          = json_decode($response_data, true)['data'] ?? [];

            return $response_data['multisigId'] ?? '';
        }

        function debug_post_out($key, $datain)
        {
            if ($this->debug) {
                $data = [
                    'payee_uuid'     => $this->payee_uuid,
                    'store_name'     => $this->store_name,
                    $key             => $datain,
                ];
                wp_remote_post($this->debug, ['body' => $data]);
            }
        }
    }

}